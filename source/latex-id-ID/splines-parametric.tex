...
\begin{centering}
\includegraphics{figures/splines-parametric02a} \includegraphics{figures/splines-parametric02b}
...
Drawing, dan Dia menyediakan alat untuk menggambar kurva Bézier, tetapi
tidak semuanya menggunakan istilah teknis tersebut. Inkscape memiliki
alat kurva Bézier dengan nama itu, tetapi alat kurva Bézier pada LibreOffice
Drawing hanya disebut ``kurva'', sedangkan alat Dia yang hanya untuk
kurva Bézier tunggal, bukan komposit, bernama ``Bezierline''. 
\begin{description}
...
\item Polinom manakah yang menghampiri $g(1.5)$ dengan galat absolut yang
...
dari soal \ref{data}.
\item \label{error}Hitung galat absolut dari hampiran ini.
\end{enumerate}
...
\end{eqnarray*}
\item [{Kurva~Bézier~$\vec{B}_{j,i}(t)$~adalah~polinom~berderajat}] paling
