...
galat mutlak ketika menggunakan $T_{n}(x)$ untuk menghampiri $f(x)$
...
\item \label{enu:taylorObservation2}Galat mutlak ketika menggunakan $T_{n}(x)$
...
\par\end{centering}
\end{figure}
 Karena $|R_{n}(x)|$ mengukur secara tepat galat mutlak $|T_{n}(x)-f(x)|$,
...
ingin menghampiri $\ln(x)$ dengan galat mutlak tidak lebih dari $0.1$.
Karena $|\xi^{-3}|$ dan $|\xi^{-12}|$ merupakan fungsi menurun terhadap
$\xi$, keduanya mencapai nilai maksimum pada interval tertutup di
titik ujung kiri interval tersebut. Oleh karena itu, untuk $x\geq e^{2}$,
...
\item Tentukan suatu batas atas bagi galat mutlak hampiran tersebut dengan
...
\item Tentukan batas atas teoretis bagi galat mutlak setiap hampiran pada
bagian (c).
\item Tentukan batas bawah teoretis bagi galat mutlak setiap hampiran pada
bagian (c).
\item Tentukan galat mutlak sebenarnya untuk setiap hampiran pada bagian
...
$x\in[0,10]$. Tentukan batas bawah dan batas atas bagi galat mutlak
...
mutlak ketika $T_{4}(x)$ digunakan untuk menghampiri $f(x)$.
