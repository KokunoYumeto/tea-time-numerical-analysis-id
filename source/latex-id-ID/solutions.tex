...
Menurut teorema Taylor, galat mutlak $|f(x)-T_{2}(x)|=|R_{2}(\xi)|$
...
galat mutlak, kita juga dapat menentukan batas bawah. Analisis yang
...
Untuk setiap nilai $x$, kita harus memaksimumkan nilai mutlak dari
...
\item [{(ii)}] Kita mencari ekstrem mutlak dari $f$ pada $[-4,-.9]$.
...
\item [{(iii)}] Kita mencari ekstrem mutlak dari $f'$ pada $[-4,-.9]$.
...
|f(1.4)-P_{1}(1.4)| & \leq & .015\max_{x\in[1.25,1.6]}|\pi^{2}\sin\pi x|\\
 ...
 & < & .149
\end{eqnarray*}
Galat mutlak sebenarnya adalah $|f(1.4)-P_{1}(1.4)|=|\sin(1.4\pi)+.8166|\approx.134$,
...
|f(1.4)-P_{2}(1.4)| & \leq & .002\max_{x\in[1,1.6]}|\pi^{3}\cos\pi x|\\
 ...
 & < & .0620
\end{eqnarray*}
Galat mutlak sebenarnya adalah $|f(1.4)-P_{2}(1.4)|=|\sin(1.4\pi)+.918232|\approx.0328$,
...
(ii) Galat mutlak adalah nilai mutlak selisih antara hampiran dan
...
dan $f'(4)=-25$. Galat mutlaknya adalah
\begin{eqnarray*}
...
\end{eqnarray*}
Jadi, sesuai perkiraan, galat mutlak pada hampiran $f'(2)$ lebih
kecil daripada galat mutlak pada $f'(4)$, tetapi perbandingan galat
...
\]
yang memiliki galat mutlak $\frac{3}{8}\pi$. Karena Aturan Trapesium
...
$.1\%$, atau yang lebih kecil, maupun apakah galat mutlak lebih penting.
Tanpa konteks tersebut, kita cukup menggunakan representasi visual,
yang menunjukkan bahwa titik-titik hampiran sangat dekat dengan grafik
solusi eksak, lalu menyimpulkan bahwa hampiran tersebut merupakan
...
suku $h^{3}$, kita menyimpulkan bahwa metode ini memiliki galat pemotongan
lokal $O(h^{3})$. Rumus integrasi yang menjadi asal penurunannya
memiliki galat pemotongan lokal $O(h^{4})$, sehingga akurasinya sebagai
pemecah PDB sedikit lebih rendah. Namun, galat pemotongan lokal $O(h^{3})$
selaras dengan laju kekonvergenan $O(h^{2})$ yang ditentukan secara
eksperimental. Bahkan, galat pemotongan lokal inilah yang menghasilkan
laju$O(h^{2})$ kekonvergenan tersebut.
...
(ordennya) adalah 3 dan karena itu galat pemotongan lokalnya $O(h^{4})$.
...
metode dengan toleransi $10^{-4}$, tetapi ini hanyalah galat pemotongan
...
sempurna dengan galat pemotongan, Gambar 
\begin{figure}
...
atau menggunakan galat relatif alih-alih galat mutlak. Apa pun caranya,
...
Jika galat global berskala sempurna dengan galat pemotongan, Gambar
\begin{figure}
...
galat relatif alih-alih galat mutlak. Apa pun caranya, persoalan ini
menambahkan lapisan kompleksitas lain pada metode.
\end{description}
